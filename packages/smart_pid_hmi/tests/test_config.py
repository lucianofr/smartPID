...
def _reload_config(
    ...
    Also restores the original ``smart_pid_hmi.config`` entry in
    ``sys.modules`` after the test so later tests (e.g. in ``tests/hmi/``)
    that hold module-level function references do not end up bound to a
    reloaded module's ``__globals__``.
    """
    ...
    # Save the original module so we can reinstate it on teardown — monkeypatch
    # does not track sys.modules.pop(), so without this finalizer the reloaded
    # module would linger and pollute unrelated tests.
    original = sys.modules.get("smart_pid_hmi.config")

    def _restore() -> None:
        if original is not None:
            sys.modules["smart_pid_hmi.config"] = original
        else:
...
def test_user_config_dir_on_windows(request, monkeypatch, tmp_path):
    monkeypatch.setattr(sys, "platform", "win32")
    appdata = tmp_path / "AppData" / "Roaming"
    appdata.mkdir(parents=True)
    cfg = _reload_config(request, monkeypatch, tmp_path, appdata)
    # noqa: SIM300 is needed — ruff reads ``path / "str" / "str"`` as literal-like.
    assert cfg.USER_CONFIG_FILE == appdata / "SmartPID" / "hmi.env"  # noqa: SIM300


def test_user_config_dir_on_linux(request, monkeypatch, tmp_path):
    monkeypatch.setattr(sys, "platform", "linux")
    cfg = _reload_config(request, monkeypatch, tmp_path, appdata=None)
    assert cfg.USER_CONFIG_FILE == tmp_path / ".smart-pid" / "hmi.env"  # noqa: SIM300


def test_user_config_overrides_packaged(request, monkeypatch, tmp_path):
...
def test_packaged_defaults_used_when_no_user_config(request, monkeypatch, tmp_path):
    monkeypatch.setattr(sys, "platform", "linux")
    cfg = _reload_config(request, monkeypatch, tmp_path, appdata=None)
