...
| E2E-006 | Executive dashboard from the top bar | PASS | `E2E-006-executive-nav.png` | Repurposed: the command palette was removed; the number now covers the navigation path that replaced it |
...
| E2E-036 | Executive KPIs | PASS | `E2E-036-executive-kpis.png` | Steps restated: `/executive` is opened from the top bar. Expected outcome unchanged. Feature added earlier: /system/status publishes CPU and memory (psutil, soft dep) |
...
| E2E-045 | Theme persistence | PASS | `E2E-045-themes.png` | Procedure re-specified for the fourth theme: four themes persist across reload; Neon is the fresh-profile default and paints pre-mount. **Bug found and fixed** (this task): a real full-page reload against the live backend raced `AuthContext`'s hook-wiring effect against a descendant's first authenticated fetch — React commits effects bottom-up, so a child's mount-time request could fire before `setAuthHooks` registered the live token, draw a real (but spurious) 401, and get logged out once the real `onUnauthorized` handler wired up moments later. Fixed by registering the api↔auth hooks synchronously in `AuthProvider`'s render body instead of a `useEffect`. Re-verified against the **real** FastAPI daemon/WebSocket with no route mocking (Chromium via Playwright's CDP, per this runbook's contract): 5 consecutive real reloads all kept the session; the theme cycle above survived every reload authenticated; a fresh browser context (no localStorage) opened on `neon` pre-mount with no other theme ever recorded on `<html>`. |
| E2E-046 | Halo/migration | PASS | `E2E-046-halo-migration.png` | Procedure re-specified: the halo follows `--glow-trace`, so it is present in Phosphor **and** Neon and absent in Recorder and ISA-101. Strictly more specific than the old "only in Phosphor". Re-run against the real backend (see the E2E-045 auth-race fix above, which this procedure's `ocean`→reload step also depends on): `--glow-trace`/`data-glow` confirmed per theme (Recorder 0px/off, Phosphor 4px/on, ISA-101 0px/off, Neon 8px/on) on the real dashboard's live trend, legacy `ocean` still migrates to Recorder and survives a real reload, and a live forced CRITICAL alarm under Neon (real backend, real alarm-config PUT) showed the bloom on exactly the row/badge/focus-ring/hover surfaces with nothing else lit — including a live `<Badge tone="warn">` MAN-mode badge, confirmed to genuinely compose the `badge-glow` box-shadow (`test-evidence/E2E-046-badge-glow-live.png`). |
